...
static void layer_update_callback(Layer *me, GContext* ctx) {
  ...
  GRect bounds = image->bounds;
	...
	//centre image
  graphics_draw_bitmap_in_rect(ctx, image, (GRect) { .origin = { (144 - bounds.size.w) / 2, 0 }, .size = bounds.size});
}
...
void handle_tap(AccelAxisType axis, int32_t direction) {
	  APP_LOG(APP_LOG_LEVEL_DEBUG, "tap! axis %d direction %d ", (int)axis,(int)direction);
	if (show_date == 0){
	   displayDate(NULL);
